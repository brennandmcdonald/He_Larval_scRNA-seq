...
he.integrated <- FindClusters(he.integrated, resolution = 3) #57 clusters with resolution=3
...
# Cluster UMAP
DimPlot(he.integrated, reduction = "umap", label = TRUE)

# Timepoint UMAP
DimPlot(he.integrated, reduction = "umap", group.by = "group", label = TRUE, repel = TRUE)
```
...
# Figure 2 time umap
# Export as 6x8 for figure 2
DimPlot_scCustom(he.integrated,
                 group.by = "time_int",
                 colors_use = viridis_plasma_dark_high[x]) +
  theme(legend.position = "none")

# Figure 2 cell type umap
# Export as 8.25x11 for figure 2
DimPlot_scCustom(he.integrated,
                 group.by = "celltype",
                 colors_use = "alphabet") +
  theme(legend.position = "none")
...
write.csv(he.markers.save, file = "6-60hr\ Integrated\ UMAP/HE_6-60hr_cluster-markers.csv")

top10markers <- he.integrated.markers %>%
  group_by(cluster) %>%
...
write.csv(top10markers, file = "6-60hr\ Integrated\ UMAP/HE_6-60hr_celltype-markers.csv")
```

Make dotplots for supplement
...
levs <- factor(c("Early Ectoderm",
                 "Set Aside", # Make sure to change to undifferentiated in the PDF
                 "Animal Pole Domain",
                 "Larval Ectoderm",
                 "Early Endomesoderm",
                 "Early Mesoderm",
...
cell_counts <- he.integrated@meta.data %>% 
  group_by(celltype, Stage) %>% 
  summarize(n = n())

pivot_wider(cell_counts, names_from = Stage, values_from = n, values_fill = 0) %>% View()
```
